...
public class SkyLineDrawing {
	 ...
	 * @return
	 * 
	 * A key point is the left end point of a horizontal line segment.
	 */
	private List<int[]> getSkyline(int[][] buildings) {
		List<int[]> resultLi = new ArrayList<>();
		
		List<Building> buildingsLi = Arrays.stream(buildings).flatMap(building->
					                 Stream.of(new Building(building[0],true,building[2]),
					                      new Building(building[1],false,building[2])))
				                     .sorted()
